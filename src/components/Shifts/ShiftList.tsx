...
// import { AppDispatch } from "../../redux/store"; // Add this
...
//     return schedule;
//   };
//   // ✅ FETCH DEPARTMENTS ON MOUNT (for admin)
//   useEffect(() => {
//     const loggedInUser = localStorage.getItem("loggedInUser");
...
//   //     // const response = await dispatch(fetchDepartments()).unwrap();
//   //     // setDepartments(response);

//   //     // For now, extract from employees as fallback
//   //     const uniqueDepts = [
//   //       ...new Set(employees.map((emp) => emp.specificRole).filter(Boolean)),
//   //     ].sort();
...
//       // Replace with your actual dispatch
//       const result = await dispatch(assignShiftAsync(data)).unwrap();
//       message.success(
//         `Shift assigned to ${selectedEmployees.length} employee(s) successfully!`
//       );
//       setIsModalVisible(false);
...
//   //       } else {
//   //         console.log("User is Employee, searching for employee data...");

//   //         // Try to find employee by email (trim whitespace and lowercase for comparison)
//   //         const employee = employeesData.find(
//   //           (emp: EmployeeData) =>
//   //             emp.email?.toLowerCase().trim() ===
//   //             userData.email?.toLowerCase().trim()
...
//   //         if (employee) {
//   //           setCurrentEmployee(employee);
//   //           console.log("Current Employee Set:", employee);
//   //         } else {
//   //           console.log("Employee NOT found with email:", userData.email);
//   //           console.log(
//   //             "Available emails:",
//   //             employeesData.map((e: EmployeeData) => e.email)
//   //           );

//   //           // FALLBACK: If not found by email, try to create a temp employee object from userData
//   //           if (userData.email) {
...
//   //             };
//   //             setCurrentEmployee(tempEmployee);
//   //             console.log("Using temp employee:", tempEmployee);
//   //           }
//   //         }
//   //       }
//   //     } else {
...
//   //     message.success(
//   //       `Shift assigned to ${result.created} employee(s) successfully!`
//   //     );
//   //     setIsModalVisible(false);
//   //     form.resetFields();
//   //     setSelectedEmployees([]);
...
//   // // Replace handleSubmitEmployeeRequest
//   // const handleSubmitEmployeeRequest = async (values: any) => {
//   //   if (!currentEmployee) {
//   //     message.error("Employee data not found!");
//   //     return;
...
//           {!isAdmin && (
//             <Card style={{ marginTop: 20, background: "#fff3cd" }}>
//               <Text strong>Debug Info:</Text>
//               <br />
//               <Text>Is Admin: {isAdmin ? "Yes" : "No"}</Text>
//               <br />
//               <Text>
//                 Current Employee: {currentEmployee ? "Found" : "NOT FOUND"}
//               </Text>
//               <br />
//               {currentEmployee && (
//                 <>
...
//                   <br />
//                   <Text>Role: {currentEmployee.specificRole}</Text>
//                 </>
//               )}
//             </Card>
//           )} */}
...
//                         {days.map((day) => {
//                           const schedule = getEmployeeWeekSchedule();
//                           const shift = schedule[day] || currentEmployee.shift;
//                           return (
//                             <td
//                               key={day}
//                               style={{
...
const ShiftManagement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (loggedInUser) {
      ...
      if (userData.userRole === "admin") {
        ...
        setCurrentEmployee(employee);
        // ✅ FETCH EMPLOYEE'S SHIFT ASSIGNMENTS (so they can see admin-assigned shifts)
  ...
  // ✅ ADD THIS NEW EFFECT - Update current employee when assignments change
  useEffect(() => {
    if (!isAdmin && currentEmployee && assignments.length > 0) {
      // Find if there's an active assignment for the current employee
      const now = new Date();
      const activeAssignment = assignments.find((assignment) => {
        const startDate = dayjs(assignment.startDate);
        const endDate = dayjs(assignment.endDate);
        const checkDate = dayjs(now);
      ...
      // If there's an active assignment, update the employee's shift
      if (activeAssignment) {
        setCurrentEmployee((prev) => ({
          ...prev!,
          shift: activeAssignment.shift,
          customShift: activeAssignment.customShift,
        }));
      }
    }
  }, [assignments, isAdmin, currentEmployee?.email]);
